...
public interface SolutionReportRepository extends JpaRepository<SolutionReport, SolutionReportId> {
    ...
    @Query("SELECT sr FROM SolutionReport sr WHERE sr.solutionId.id = :solutionId")
    ...
    @Query("SELECT sr FROM SolutionReport sr WHERE sr.studentId.id = :studentId")
    ...
    @Query("SELECT sr FROM SolutionReport sr WHERE sr.studentId.id = :studentId AND sr.solutionId.id = :solutionId")
    ...
    @Query("SELECT CASE WHEN COUNT(sr) > 0 THEN true ELSE false END FROM SolutionReport sr WHERE sr.solutionId.id = :solutionId")
    boolean existsBySolutionId(@Param("solutionId") UUID solutionId);

    // Count reports by student using clean method name
    @Query("SELECT COUNT(sr) FROM SolutionReport sr WHERE sr.studentId.id = :studentId")
    long countByStudentId(@Param("studentId") UUID studentId);

    // Find reports with best performance by student (least time taken)
    @Query("SELECT sr FROM SolutionReport sr WHERE sr.studentId.id = :studentId ORDER BY sr.timeTaken ASC")
    ...
    @Query("SELECT sr FROM SolutionReport sr WHERE sr.studentId.id = :studentId ORDER BY sr.memoryUsed ASC")
    ...
    @Modifying
    @Query("DELETE FROM SolutionReport sr WHERE sr.solutionId.id = :solutionId")
    ...
    @Modifying
    @Query("DELETE FROM SolutionReport sr WHERE sr.studentId.id = :studentId")
