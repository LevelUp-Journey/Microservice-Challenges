...
public interface SolutionRepository extends JpaRepository<Solution, SolutionId> {
    ...
    @Query("SELECT s FROM Solution s WHERE s.studentId.id = :studentId AND s.codeVersionId.id = :codeVersionId")
    ...
    @Query("SELECT s FROM Solution s WHERE s.studentId.id = :studentId")
    List<Solution> findByStudentId(@Param("studentId") UUID studentId);

    // Find all solutions by challenge ID using clean method name
    @Query("SELECT s FROM Solution s WHERE s.challengeId.id = :challengeId")
    ...
    @Query("SELECT CASE WHEN COUNT(s) > 0 THEN true ELSE false END FROM Solution s WHERE s.studentId.id = :studentId AND s.codeVersionId.id = :codeVersionId")
    ...
    @Query("SELECT COUNT(s) FROM Solution s WHERE s.studentId.id = :studentId")
    long countByStudentId(@Param("studentId") UUID studentId);

    // Count solutions by challenge using clean method name
    @Query("SELECT COUNT(s) FROM Solution s WHERE s.challengeId.id = :challengeId")
